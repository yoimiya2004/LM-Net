...
def check_image_size(x,window_size=4):
    ...
    mod_pad_w = (window_size  - w % (window_size)) % (
                window_size)
    x = F.pad(x, (0, mod_pad_w, 0, mod_pad_h), 'reflect')  # (left, right, top, bottom)
...
def main():
    ...
    parser.add_argument('--max_size', type=int, default=4800,
    ...
    for idx, path in enumerate(paths):
        ...
        gt_path = args.gt
        file_name = path.split('/')[-1].split('_')[0] + '.png'  # haze4k
        # file_name = path.split('/')[-1]  # reside_6k
        # file_name = path.split('/')[-1].replace('hazy', 'GT')
        ...
        # ssim = ssim_(output_img, gt_img, multichannel=True)
        psnr = psnr_(output_img, gt_img)
