...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if (x.shape[-2] % 2 != 0) or (x.shape[-1] % 2 != 0):
            print(f"[DWT DEBUG] odd size before DWT: {tuple(x.shape)}")
        ...
        x_3 = self.layer_3(x4, blocks[-1])
        x_3 = self.up_group4(x_3)

        x_2 = self.layer_2(x_3, blocks[-2])
        x_2 = self.up_group3(x_2)

        x_1 = self.layer_1(x_2, blocks[-3])
        x_1 = self.up_group2(x_1)

        x_0 = self.layer_0(x_1, blocks[-4])
