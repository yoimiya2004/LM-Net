...
class FeMaSRModel(BaseModel):
    ...
    def test(self):
        ...
        if h * w < min_size:
            # 整图推理
            out = net_g(lq_input)
            self.output = out[-1] if isinstance(out, (list, tuple)) else out
        else:
            # 分块推理（大图像）
    ...
    def dist_validation(self, dataloader, current_iter, epoch, tb_logger, save_img, save_as_dir=None):
        """DDP validation: only run on rank0 to avoid duplicate validation/saving."""
        logger = get_root_logger()
        if dist.is_available() and dist.is_initialized():
            if dist.get_rank() != 0:
                return
        logger.info('DDP validation: only rank0 runs validation.')
        self.nondist_validation(dataloader, current_iter, epoch, tb_logger, save_img, save_as_dir)

    def nondist_validation(self, dataloader, current_iter, epoch, tb_logger, save_img, save_as_dir = None):
        """非分布式验证逻辑：计算指标、保存结果、更新最佳模型"""
        dataset_name = 'NTIRE2026'  # 验证数据集名称
